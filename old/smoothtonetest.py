...
def make_wave(freq=440, time=1, amp=100, phase=0, samplerate=44100, bitspersample=16):
    ...
    for i in range(int(samplerate*time)):
        ...
        bytelist.append(amp*int(round(math.e**(-(2*i)/(samplerate*time))*(2**(bitspersample - 1) - 1)*cello)))
        incadd += increment
    return bytelist
...
def songToWave(song):
    ...
    result=[]
    voices=[]
    global last,last2,tempo,amp,count,totalLen,progress
    totalLen=len(song)*len(song[0])
    for voice in song:
        tempo=tempo0
        amp=amp0
        print('v',voice)
        ...
        for chord in voice:
            print('c',chord)
            count+=1
            if (100*count/totalLen)//100>progress:
                progress+=1
                print('|',end='')
            chordWave=[]
...
play(clairDeLune,'clair de lune smoothed')