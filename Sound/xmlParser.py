...
def Parse(path):
    ...
    try:
        ...
        tempo=TEMPO#int(input('What is the tempo of the piece?'))
    completeSong={}
    for part in xmldoc.getElementsByTagName('part'):
        ...
        completeSong[instrument]=parseInstrument(part)
    result=[]
...
def parseInstrument(part):
    print(part.getAttribute('id'))
    if part.getElementsByTagName('transpose')!=[]:
    ...
    noOfBeats=int(part.getElementsByTagName('beats')[0].childNodes[0].nodeValue)
    beatType=beatTypes[part.getElementsByTagName('beat-type')[0].childNodes[0].nodeValue]
    ...
    while i <len(measures):
        ...
        for note in notes:
            flag=False
            # ignore grace notes
            if (note.getElementsByTagName('grace')!=[] or 
                note.getElementsByTagName('notehead')!=[] and 
                note.getElementsByTagName('notehead')[0].childNodes[0].nodeValue=='slash'):
                continue
            # for notes
            ...
            elif note.getElementsByTagName('unpitched')!=[]:
                pitch='p'
                octave='1'
            else:
            # for rests
...
def connectTied(ties,note,time,voice):
    ...
    (pitch,octave)=note
    try:
        if (pitch+'b',octave) in ties[voice]:
            note=(pitch+'b',octave)
        elif (pitch+'#',octave) in ties[voice]:
            note=(pitch+'#',octave)
        elif (pitch[0],octave) in ties[voice]:
            note=(pitch[0],octave)
    except:
        for voice in ties:
            for tie in ties[voice]:
...
skipPercussion=False
TEMPO=115

testLengths('pmat.xml')

#print(Parse('ssbb.xml'))