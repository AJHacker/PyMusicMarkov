...
class Circle(object):
    def __init__(self,note,octave,duration,tempo,data):
        ...
        self.y = data.height/10 * (8-octave) -((ord(note[0])-97)%97)*(data.width/10 /7)
...
def timerFired(data):
    if(data.screen == 'Visualize'):
        print(len(data.dots),len(data.songWithPositions))
        data.pointInSong +=(data.timerDelay/1000)*(data.tempo/60)*data.beatUnit * 2
        #print(data.pointInSong)
        t = threading.Thread(target=makeCircles,args=(data,))
        t.start()
...
def playSong(pathtowav):
    x = subprocess.call(["afplay", pathtowav])
...
def drawMainScreen(canvas,data):
    ...
    def getvals():
        #print(data.buttonsdown)
        for i in data.buttonsdown:
            #print(data.searchresults[i])
